...
ssize_t virt_uart_buf_read(struct file* file,
                           char __user* buf,
                           size_t       size,
                           loff_t*      ppos)
{
...
static ssize_t virt_uart_buf_write(struct file*       file,
                                   ...
                                   loff_t*            off)
{
    ...
    ssize    = (size > BUF_LEN) ? BUF_LEN : size;
    ret      = copy_from_user(rxbuf, buf, ssize);
    rx_buf_w = ssize;
...
static void virt_set_termios(struct uart_port* port,
