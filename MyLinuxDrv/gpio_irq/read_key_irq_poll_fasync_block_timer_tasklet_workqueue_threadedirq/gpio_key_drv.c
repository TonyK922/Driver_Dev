...
{
    // printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
    struct gpio_key* gpio_keyp = container_of(work, struct gpio_key, work);

    int val;

    val = gpiod_get_value(gpio_keyp->gpiod);

    printk("%s process %s  pid %i \n", __FUNCTION__, current->comm,
           current->pid);
    printk("%s key %d  val %d \n", __FUNCTION__, gpio_keyp->gpio, val);
}
static irqreturn_t gpio_key_thread_fn(int irq, void* data)
{
    struct gpio_key* gpio_key = data;
    int              val;
...
static int gpio_key_probe(struct platform_device* pdev)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        */
        ...
        INIT_WORK(&gpio_keys[i].work, key_work_func);
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        err = request_threaded_irq(gpio_keys[i].irq, gpio_key_irq,
                                   gpio_key_thread_fn,
                                   IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
                                   "my_gpio_key", &gpio_keys[i]);
    }
